...
§
agentic-os: /root/code/agentic-os, Cloudflare Worker. Worker: tanstack-start-app.lighthousegrouptr.workers.dev, custom domain: agentic.lighthousegroup.net.tr. Deploy: wrangler deploy (uses dist/server/wrangler.json, NOT wrangler.jsonc). Must edit dist/server/index.js directly via scripts/build-index.cjs. APP_JS as external file (/app.js endpoint) - inline scripts break in JS string literals. XHR sync works, async fetch hangs (CF zaraz monkey-patching). Build pipeline: aggregate.ts -> live-data.json -> transform-live-data.ts -> live-data-legacy.json -> build-index.cjs -> dist/server/index.js -> wrangler deploy. KV: live-data (legacy format). STALE_DAYS=30. Cron 2655c3b31f43 auto-deploys. Dashboard working as of 2026-06-03. Cloudflare edge cache may cause stale HTML for users - purge via dashboard or add ?v= param.