...
- Mount `~/.claude:/root/.claude:ro` and re-run aggregate at runtime via the start command
...
3. **Solutions** (pick one):
   ...
   - **Serve from Caddy**: Add `handle /__live-data { root * /app/src/data/; rewrite * /live-data.json; file_server }` to Caddyfile (NOTE: `handle` not `handle_path` — see "Overwriting Caddyfile" section), mount `~/.claude` as a volume, and run aggregate in the start command or via cron.
