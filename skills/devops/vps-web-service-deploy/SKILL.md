...
**Bare `wrangler deploy` confirmed working** at r33 and r36 (wrangler v4.86.0). The `--outdir dist/client` flag is functionally ignored — wrangler reads `dist/server/wrangler.json` generated by the Vite plugin at build time. Both bare `wrangler deploy` and `npx wrangler deploy --outdir dist/client` produce identical deploys. Prefer bare `wrangler deploy` (system-installed at `/usr/bin/wrangler`).
| r34 | v4.90.0 | — |
| r32 | v4.86.0 | — |
| r31 | v4.86.0 | — |
| r30 | v4.86.0 | — |
...
Non-critical wrangler updates do not affect deploy success.

**⚠️ TanStack Start SSR app (agentic-os) deploy command:**
```bash
...
Bare `wrangler deploy` works for agentic-os because `wrangler.jsonc` at the project root configures `main: dist/server/index.js` and `compatibility_flags: [nodejs_compat]`. The build produces `dist/server/index.js` and `dist/client/` assets, and wrangler uploads them correctly without needing `--outdir`.

**Do NOT use `--outdir dist/client`** — this flag was a previous workaround and is no longer needed. The `@cloudflare/vite-plugin` handles asset upload through `dist/server/wrangler.json` generated at build time.
...
source /root/.profile 2>/dev/null
# or
export CLOUDFLARE_API_TOKEN=$(grep -oP 'CLOUDFLARE_API_TOKEN=\K.*' /root/.profile)
wrangler deploy
```

Also: **`bun run deploy` does NOT exist** in `package.json` — there is no `deploy` script. Must invoke wrangler directly.

For non-TanStack Start Workers projects, bare `wrangler deploy` (on PATH at `/usr/bin/wrangler`) works fine.
...
- **`wrangler deploy` for agentic-os**: Use bare `wrangler deploy` (NOT `npx wrangler deploy --outdir dist/client`). The `--outdir` flag is ignored for Vite-based TanStack Start projects — wrangler reads `dist/server/wrangler.json` generated at build time, which already points to the correct `main` and asset upload paths. Using `npx` inside the VPS container invokes a potentially different wrangler version; the system-installed `/usr/bin/wrangler` (currently v4.86.0) is the canonical one. See the "wrangler version tracking" section above.
...
For **TanStack Start SSR** apps (agentic-os):
```bash
wrangler deploy     # bare command works; --outdir is ignored for Vite projects
```

For **other Workers** projects:
```bash
wrangler deploy     # bare command works for standard Workers
```
...
|---|---|---|
| Deploy command | `wrangler deploy` (bare) | Auto on git push |
...
npx wrangler deploy --outdir dist/client
```
...
Even after `rm -rf dist && bun run build && npx wrangler deploy --outdir dist/client`, wrangler may report **"No updated asset files to upload"** and skip uploading the new client assets. The old assets remain in Cloudflare's KV, and the worker serves stale chunks — including a stale `worker-entry` that returns placeholder responses like `"building..."`.
...
- `references/agentic-os-version-log.md` — Full deploy history (r1–r36): Version IDs, file counts, build times
...
- `references/2026-06-02-cron-run-full-refresh-deploy-r35.md` — Run 35: `--outdir dist/client` required, `bun run deploy` doesn't exist, bare `wrangler deploy` fails
