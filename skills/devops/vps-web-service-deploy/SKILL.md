...
**IMPORTANT (2026-06-02):** TanStack Start 1.167+ broke SSR — `@tanstack/react-start/server-entry` was removed. The Vite plugin silently produces a "placeholder" handler in `dist/server/index.js`. 

**For agentic-os specifically:** The project has a `dist/client/dashboard.html` (static HTML dashboard) that works without JS bundles. The 3D memory graph lives in the TanStack Start SPA which requires `index.html` (not generated by the broken build). Current recommendation: use the static `dashboard.html` as the primary interface, or deploy via Docker+Caddy for the full SPA. See `references/tanstack-start-1167-server-entry-removed.md`.

**Do NOT use `--outdir dist/client`** — this flag was a previous workaround and is no longer needed.

**`CLOUDFLARE_API_TOKEN` is required for wrangler deploy.** On this VPS, the token is stored in `/root/.profile` (NOT `.bashrc`, NOT `.env.local`). In non-interactive/cron sessions, you must source it before deploying:
```bash
source /root/.profile 2>/dev/null
wrangler deploy
```

Also: **`bun run deploy` does NOT exist** in `package.json` — there is no `deploy` script. Must invoke wrangler directly.
